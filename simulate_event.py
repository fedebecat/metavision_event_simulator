...
def parse_args(only_default_values=False):
    ...
    parser.add_argument('--height', default=1024, type=int, help="frame height")
    parser.add_argument('--width', default=2048, type=int, help="frame width")
...
def test_gpu_simulator(path,
    ...
    last_images = None
    start = time.time()
    record_video = True #override argparse
    if record_video:
        out = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 5, (width, height))
    for i, batch in enumerate(dl):
        ...
        final = im # np.concatenate((im, imin), axis=1)
        if record_video:
            out.write(final)
        last_images = images
        start = time.time()
